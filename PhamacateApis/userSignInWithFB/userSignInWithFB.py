# -*- coding: utf-8 -*-
"""
Created on Fri Jun 24 18:18:58 2016
...
def handler(event, context):
	...
	if event['jsonB']['Queries'].has_key('USER_NAME'):
		...
		if len(rows) > 0:
			...
			updateQuery += "FB_ID = '%s', " %(event['jsonB']['Queries']['FB_ID'])
			...
			insertQuery += "(USER_NAME, TOKEN, TYPE, FIRST_NAME, LAST_NAME, FB_ID, IMAGE_LINK, DEVICE_TOKEN)" + " VALUES ('%s', '%s', 'FB', '%s', '%s', '%s', '%s', '%s')" %(userName, token, event['jsonB']['Queries']['FIRST_NAME'], event['jsonB']['Queries']['LAST_NAME'], event['jsonB']['Queries']['FB_ID'], event['jsonB']['Queries']['IMAGE_LINK'], event['jsonB']['Queries']['DEVICE_TOKEN'])
		...
		searchQuery = "SELECT USER_NAME FROM pharmacateDB.USER "
		searchQuery += "WHERE FB_ID = '%s'" % (event['jsonB']['Queries']['FB_ID'])
		
		dictionary = {"FB_ID":event['jsonB']['Queries']['FB_ID'], "TIME_STAMP": millis, 'exp': datetime.datetime.utcnow() + datetime.timedelta(seconds=tokenDuration)}
		token = jwt.encode(dictionary, secretKey, algorithm='HS256')
		...
		if len(rows) > 0:
			...
			updateQuery += "IMAGE_LINK = '%s', " %(event['jsonB']['Queries']['IMAGE_LINK'])
			updateQuery += "DEVICE_TOKEN = '%s' " %(event['jsonB']['Queries']['DEVICE_TOKEN'])
			updateQuery += "WHERE FB_ID = '%s' " % (event['jsonB']['Queries']['FB_ID'])
			...
			insertQuery = "INSERT INTO pharmacateDB.USER "
			insertQuery += "(TOKEN, TYPE, FIRST_NAME, LAST_NAME, FB_ID, IMAGE_LINK, DEVICE_TOKEN)" + " VALUES ('%s', 'FB', '%s', '%s', '%s', '%s', '%s')" %(token, event['jsonB']['Queries']['FIRST_NAME'], event['jsonB']['Queries']['LAST_NAME'], event['jsonB']['Queries']['FB_ID'], event['jsonB']['Queries']['IMAGE_LINK'], event['jsonB']['Queries']['DEVICE_TOKEN'])
