...
{
    BOOL isAddNewCatPressed;
    BOOL isLoactionPressed;
...
-(void) viewWillAppear: (BOOL)animated
{
    ...
    NSLog(@"appear categoryArray %d",categoryArray.count);
    
    NSLog(@"Image size - %f %f ", imgView.image.size.width, imgView.image.size.height);
    
    NSLog(@"total photo %d",[[DataBaseManager getInstance] returnCount]);
...
//    categoryField.rightViewMode = UITextFieldViewModeAlways;
    ...
    systemVer = [[UIDevice currentDevice] systemVersion];
}
...
{
    isLoactionPressed = YES;
    isAddNewCatPressed = NO;
...
-(void)submitButtonPressed:(id)sender
{
    ...
    {
        appDelegate.globalCategoryIndex = tempGlobalInedx;
    }
...
//            }
        ...
        {
            if([[DataBaseManager getInstance] returnCount] == 3 || ([[DataBaseManager getInstance] returnCount] % 9 == 0))
            {
...
{
    isAddNewCatPressed = YES;
    isLoactionPressed = NO;
    if([ApplicationPreference isProUpgradePurchaseDone])
    {
...
-(void)canPurchaseNow
{
    ...
    if ([[InAppPurchaseManager getInstance] canMakePurchases])
    {
        ...
        purchaseAlert = [[UIAlertView alloc] initWithTitle:@"Upgrade to Plane Spotting Plus?" message:nil delegate:self cancelButtonTitle:@"Not Now" otherButtonTitles:@"Upgrade Now", @"Restore Purchases", nil];
        [purchaseAlert show];
    }
    else
    {
...
-(void)paymentSucceeded
{
    ...
    if(isAddNewCatPressed)
    {
        isAddNewCatPressed = !isAddNewCatPressed;
        catNameTextField = [[UITextField alloc] init];
        catNameTextField.frame = CGRectMake(12.0, 45.0, 260.0, 25.0);
        catNameTextField.backgroundColor = [UIColor whiteColor];
        catNameTextField.returnKeyType = UIReturnKeyDone;
    ...
    else if (isLoactionPressed)
    {
        isLoactionPressed = !isLoactionPressed;
        [noteField resignFirstResponder];
        [categoryField resignFirstResponder];
...
- (void) alertView:(UIAlertView *)alert clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if(alert == newCatAddAlert)
    {
        ...
        if (buttonIndex == 1)
        {
            ...
            NSLog(@"clickedButtonIndex categoryArray %d",categoryArray.count);
...
{
    NSLog(@"numberOfRows categoryArray %d",categoryArray.count);
...
-(void)dropboxDidLogin
{
    ...
    NSArray *categoryArrays = [[DataBaseManager getInstance] getCatagoryNames];
    ...
    for (int i = 0; i < categoryArrays.count; i++)
    {
        ...
        if (success)
        {
            ...
            for (int j = 0; j < subCatArray.count; j++)
            {
                ...
                UIImage *imgs = [UIImage imageWithContentsOfFile:subCat.PhotoRef];
                
                [imageFile writeData:UIImagePNGRepresentation(imgs) error:nil];
            }
        }
    }
