...
//        [ALSdk initializeSdk];
    ...
    ////////////// Playhaven
//    PHPublisherOpenRequest *request = [PHPublisherOpenRequest requestForApp:PLAYHAVEN_APP_TOKEN  secret:PLAYHAVEN_APP_SECRET];
//    [request send];
    //////////////
    ...
    if([[dict objectForKey:@"ButtonState"] isEqualToString:@"Yes"])
    {
        TopAd=YES;
    }
    else
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if(buttonIndex == 1)
    {
            ...
            if(self.randomNum == 0)
            {
                if(TopAd==NO)
                {
                    [Flurry logEvent:@"Startup Upgrade Alert" withParameters:[NSDictionary dictionaryWithObjectsAndKeys:@"Startup Upgrade Popup Top",@"Name", nil]];
                    [[NSUserDefaults standardUserDefaults] setObject:@"Startup Upgrade Popup Top" forKey:ALERT_NAME];
                    [[NSUserDefaults standardUserDefaults] synchronize];
                    self.tagId=0;
                    if([[[storename objectAtIndex:self.tagId] objectForKey:@"Purchased"] isEqualToString:@"Yes"])
                    {
                        if([[[storename objectAtIndex:self.tagId] objectForKey:@"ButtonState"] isEqualToString:@"Yes"])
                        {
                            NSMutableDictionary *newDict = [[[NSMutableDictionary alloc] initWithObjectsAndKeys:[[storename objectAtIndex:self.tagId] objectForKey:@"Name"],@"Name",[[storename objectAtIndex:self.tagId] objectForKey:@"inAppIdentifier"],@"inAppIdentifier",@"Yes",@"Purchased",[NSString stringWithFormat:@"%d",self.tagId],@"tag",@"No",@"ButtonState",nil] autorelease];
                            
                            [storename replaceObjectAtIndex:self.tagId withObject:newDict];
                        ...
                        {
                            NSMutableDictionary *newDict = [[[NSMutableDictionary alloc] initWithObjectsAndKeys:[[storename objectAtIndex:self.tagId] objectForKey:@"Name"],@"Name",[[storename objectAtIndex:self.tagId] objectForKey:@"inAppIdentifier"],@"inAppIdentifier",@"Yes",@"Purchased",[NSString stringWithFormat:@"%d",self.tagId],@"tag",@"Yes",@"ButtonState",nil] autorelease];
                    ...
                    else
                    {
                        ...
                        {
                            [[InAppRageIAPHelper sharedHelper] buyProductIdentifier:[[storename objectAtIndex:self.tagId] objectForKey:@"inAppIdentifier"]];
                        }
                ...
                {
                    [Flurry logEvent:@"Startup Upgrade Alert" withParameters:[NSDictionary dictionaryWithObjectsAndKeys:@"Startup Upgrade Popup Bottom",@"Name", nil]];
                    [[NSUserDefaults standardUserDefaults] setObject:@"Startup Upgrade Popup Bottom" forKey:ALERT_NAME];
                    [[NSUserDefaults standardUserDefaults] synchronize];
                    self.tagId = 1;
                    if([[[storename objectAtIndex:self.tagId] objectForKey:@"Purchased"] isEqualToString:@"Yes"])
                    {
                    ...
                    else
                    {
                        ...
                        {
                            [[InAppRageIAPHelper sharedHelper] buyProductIdentifier:[[storename objectAtIndex:1] objectForKey:@"inAppIdentifier"]];
                        }
