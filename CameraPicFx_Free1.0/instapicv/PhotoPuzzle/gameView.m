...
- (void)viewDidLoad
{
    ...
    if([[[ALSharedData getSharedInstance] playHavenInfoObj] showOnGameover] == YES)
    {
        [self loadPlayHaven];
    }
    if([[[ALSharedData getSharedInstance] appLovinInfoObj] showOnGameover] == YES)
    {
...
//    self.navigationItem.leftBarButtonItem=[[[UIBarButtonItem alloc]initWithCustomView:editButton] autorelease];
    ...
    backButton.frame=CGRectMake(260, 3, 40, 40);
...
- (void)fbGraphCallback:(id)sender
{
    ...
    else
    {
        ...
        [variables setObject:@"I am using CameraPicFx - FREE Messenger App." forKey:@"message"];
        ...
        UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"Facebook" message:@"CameraPicFx - FREE Messenger App is posted to your wall." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
        [alert release];
...
-(void)twitterButtonTap
{
    ...
    if ([TWTweetComposeViewController canSendTweet])
    {
        ...
        [tweetSheet setInitialText:@"Lets play photo puzzle  with 'CameraPicFx - FREE Messenger App'"];
        if (Shareimage)
        {
            [tweetSheet addImage:Shareimage];
            [tweetSheet addURL:[NSURL URLWithString:@"http://georiot.co/vlq"]];
        }
...
-(void)displayComposerSheet {
 //   NSLog(@"Form fully Completed!");
    if([MFMailComposeViewController canSendMail]){
        ...
        NSString *emailString=[NSString stringWithFormat:@"<html><body><a href=\"http://georiot.co/vlq\">Lets play photo puzzle  with 'Camera Pic Fx' - FREE Messenger App'</a></body></html>"];
        
        
        [picker setMessageBody:emailString isHTML:YES]; // depends. Mostly YES, unless you want to send it as plain text (boring)
        //[picker setMessageBody:self.text isHTML:NO];
