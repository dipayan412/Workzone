...
//    self.navigationController.navigationBar.alpha = 1.0f;
    ...
    alpha = self.navigationController.navigationBar.alpha;
    
    WhosinAppDelegate *appDelegate = (WhosinAppDelegate*)[[UIApplication sharedApplication] delegate];
    
    NSArray *categoryArray = [[NSArray alloc] initWithArray:[[DataBaseManager getInstance] getCatagoryNames]];
    self.infoArray = [[DataBaseManager getInstance] getSubCat:[categoryArray objectAtIndex:appDelegate.globalCategoryIndex]];
    
    NSLog(@"gloabalCategoryIndex %d",appDelegate.globalCategoryIndex);
...
//    [tipStrip pushNavigationItem:self.navigationItem animated:NO];
    ...
    containerScrollView.frame = CGRectMake(self.view.bounds.origin.x,self.view.bounds.origin.y,self.view.bounds.size.width,self.view.bounds.size.height);
}
...
//             tipStrip.alpha = 0.0f;
             ...
             if(systemVer.floatValue >= 7.0f)
             {
//                 containerScrollView.frame = CGRectMake(0,-45,self.view.frame.size.width,self.view.frame.size.height);
             }
         }];
    }
}
...
-(void)canPurchaseNow
{
    ...
    if ([[InAppPurchaseManager getInstance] canMakePurchases])
    {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Upgrade to Autography Plus?" message:nil delegate:self cancelButtonTitle:@"Not Now" otherButtonTitles:@"Upgrade Now", @"Restore Purchases", nil];
        [alert show];
    }
    else
    {
...
-(void)paymentSucceeded
{
    ...
    else if(isTweetPressed)
    {
        isTweetPressed = !isTweetPressed;
        NSLog(@"deleted");
...
//    navController.navigationBar.alpha = 1.0f;
